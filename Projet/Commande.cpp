...
System::String^ NS_BDD::Commande::Ajouter(System::String^ nomClient, System::String^ prenomClient, System::String^ idClient, System::String^ quantite, System::String^ nomArticle, System::String^ nature, System::String^ couleur)
{
    return "Insert into Commande (reference, date_livraison, date_emission, prix_HT, prix_TTC, TVA, remise, prix_final,  id_client) select "+ this->get_reference() +", "+this->get_date_livraison()+", "+ this->get_date_emission()+", "+this->get_prix_HT()+", "+this->get_prix_TTC()+", "+this->get_TVA()+", "+this->get_remise()+", "+this->get_prix_final()+", Client.id_client from Client where(Client.nom = '"+nomClient+"' and Client.prenom = '"+prenomClient+"' and Client.id_client = '"+idClient+"') insert into Contenir(id_commande, id_article, Quantite) select Commande.id_commande, Article.id_article, '"+quantite+"' from Commande, Article where(Commande.reference = "+this->get_reference()+" and Article.nom = '"+nomArticle+"' and Article.nature = '"+nature+"' and Article.couleur = '"+couleur+"') update Commande set prix_HT = (select(Contenir.Quantite * Article.prix_HT) as michel from Contenir, Article, Commande where Contenir.id_article = Article.id_article and Contenir.id_commande = Commande.id_commande and Commande.reference = "+this->get_reference()+" and Article.nom = '"+nomArticle+"' and Article.nature = '"+nature+"' and Article.couleur = '"+couleur+"') where Commande.reference = "+this->get_reference()+" update Commande set Prix_TTC = (select(Commande.prix_HT + Commande.prix_HT * Article.TVA) as jackie from Commande, Article, Contenir where Contenir.id_commande = Commande.id_commande and Contenir.id_article = Article.id_article and Commande.reference = "+this->get_reference()+" and Article.nom = '"+nomArticle+"' and Article.nature = '"+nature+"' and Article.couleur = '"+couleur+"'), TVA = (select Article.TVA from Article, Contenir, Commande where Contenir.id_commande = Commande.id_commande and Contenir.id_article = Article.id_article and Commande.reference = "+this-> get_reference()+" and Article.nom = '"+nomArticle+"' and Article.nature = '"+nature+"' and Article.couleur = '"+couleur+"') where Commande.reference = "+this->get_reference()+" update Commande set prix_final = (select(prix_TTC + (prix_TTC * remise) / 100) from Commande where Commande.reference = "+this->get_reference()+") where Commande.reference = "+this->get_reference()+"";
}
